...
export const MismatchTokenParamsModal = ({ bridge }: Props) => {
  ...
  return (
    <ThemedModal bridge={bridge} onCancel={() => bridge.resolve(false)} paddingRem={[1, 0.5]}>
      ...
      <ModalTitle paddingRem={[1, 0, 0.2]} bold>
        {s.strings.mismatching_token_parameters}
      </ModalTitle>
      ...
      <View style={styles.alignSelfCenter}>
        <Fade visible={isChecked}>
          <SecondaryButton marginRem={[0, 0, 1.35]} label={s.strings.confirm_finish} onPress={() => bridge.resolve(true)} bold />
        </Fade>
      </View>
    </ThemedModal>
  )
