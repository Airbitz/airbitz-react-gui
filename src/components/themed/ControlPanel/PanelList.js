...
export default function PanelList(props: Props) {
  ...
  return (
    <ScrollView>
      <PanelRow title={s.strings.drawer_fio_names} route={Constants.FIO_ADDRESS_LIST} iconName="list.view" />
      <PanelRow title={s.strings.drawer_fio_requests} route={Constants.FIO_REQUEST_LIST} iconName="map" />
      <PanelRow title={s.strings.drawer_scan_qr_send} route={Constants.SCAN} iconName="scan.qr" />
      <PanelRow title={s.strings.drawer_sweep_private_key} onPress={onSweep} iconName="key" />
      <PanelRow title={s.strings.title_terms_of_service} route={Constants.TERMS_OF_SERVICE} iconName="list.box" />
      <PanelRow title={s.strings.string_share + ' ' + s.strings.app_name} onPress={onShareApp} iconName="share" />
      <PanelRow title={s.strings.settings_title} route={Constants.SETTINGS_OVERVIEW_TAB} iconName="settings" />
